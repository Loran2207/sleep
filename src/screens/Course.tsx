...
// The course wears the cosmic language in red: a starfield backdrop and
// a glowing medallion hero, so "the science of sleep" feels like part of
// the same world as the Sounds and breathing screens.
const RED = COSMIC.red.accent;
const RED_LIGHT = COSMIC.red.light;
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 190, position: 'relative', zIndex: 1 }}>
        <div style={{ padding: '0 20px 26px', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', marginTop: 10 }}>
            <CosmicMedallion hue="red" core={78}>
              <GlyphBook size={30} stroke={RED_LIGHT} />
            </CosmicMedallion>
          </div>

          <div style={{
            display: 'inline-flex', alignItems: 'center', gap: 7,
            padding: '5px 12px', borderRadius: 999, marginTop: 18,
            background: hexA(RED, 0.1),
            border: `1px solid ${hexA(RED, 0.32)}`,
            fontSize: 12, fontWeight: 500, color: RED_LIGHT,
          }}>
            <span style={{ width: 6, height: 6, borderRadius: 3, background: RED }} />
            Course · 12 lessons
          </div>
          <div style={{ fontSize: 30, fontWeight: 600, letterSpacing: '-0.025em', lineHeight: 1.1, marginTop: 14, color: '#fff' }}>
            The science<br />of sleep
          </div>
          <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.65)', marginTop: 10, lineHeight: 1.5, maxWidth: 300, margin: '10px auto 0' }}>
          ...
          <div style={{ marginTop: 22 }}>
            ...
            <div style={{ height: 5, background: 'rgba(255,255,255,0.08)', borderRadius: 3, overflow: 'hidden' }}>
              <div style={{ width: `${pct}%`, height: '100%', background: `linear-gradient(90deg, ${RED} 0%, ${hexA(RED, 0.6)} 100%)` }} />
            </div>
          </div>
        </div>

        <div style={{ padding: '6px 16px 0' }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: 'rgba(255,255,255,0.55)', padding: '0 6px 12px' }}>Lessons</div>
        {lessons.map((l, i) => {
            ...
            <div key={l.n} style={{ display: 'flex', alignItems: 'stretch', gap: 14 }}>
              <div style={{ width: 36, display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 14 }}>
                <div style={{
                  ...
                  border: isAvail ? `1.5px solid ${RED}` : `1.5px solid ${isLocked ? W.veryweak : W.ink}`,
                  background: isDone ? W.ink : (isAvail ? hexA(RED, 0.12) : W.paper),
                  color: isDone ? W.paper : (isLocked ? W.veryweak : (isAvail ? RED_LIGHT : W.ink)),
              ...
              <div onClick={() => openLesson(l)} style={{
                ...
                background: tappable ? 'rgba(255,255,255,0.04)' : 'transparent',
                border: tappable ? `1px solid ${isAvail ? hexA(RED, 0.3) : 'rgba(255,255,255,0.08)'}` : 'none',
                borderRadius: 16,
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 15, fontWeight: 500, marginTop: 3, color: isLocked ? 'rgba(255,255,255,0.6)' : W.ink, lineHeight: 1.3 }}>{l.title}</div>
                </div>
                {isAvail && (
                  <div style={{
                    width: 36, height: 36, borderRadius: 18, background: RED,
                    display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
                    boxShadow: `0 6px 18px ${hexA(RED, 0.4)}`,
                  }}><PlayIcon size={13} stroke="#fff" /></div>
                )}
                {isDone && (
...
export function Lesson() {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: W.bg, color: W.ink, fontFamily: W.font, position: 'relative' }}>
      <div style={{ position: 'absolute', inset: 0, background: '#120A0C', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <CosmicBackdrop hue="red" stars={18} glow={0.12} />
        <div style={{
          width: '80%', aspectRatio: '9 / 16', border: `2px dashed ${hexA(RED, 0.4)}`, borderRadius: 12,
          display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
          color: RED_LIGHT, fontSize: 13, gap: 8, position: 'relative', zIndex: 1,
        }}>
          <PlayIcon size={36} stroke={RED_LIGHT} />
          <div style={{ color: 'rgba(255,255,255,0.6)' }}>vertical video placeholder</div>
        </div>
      </div>
      <div style={{ position: 'relative', zIndex: 2, padding: '60px 16px 0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ flex: 1, height: 3, background: 'rgba(255,255,255,0.2)', borderRadius: 2 }}>
            <div style={{ width: `${positionPct}%`, height: '100%', background: RED, borderRadius: 2 }} />
          </div>
      ...
      <div style={{
        ...
        <div onClick={() => go('course')} style={{
          ...
          background: '#fff', color: '#000', borderRadius: 999, fontSize: 14, fontWeight: 600, cursor: 'pointer',
