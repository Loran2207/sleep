...
export function JournalEntryEdit() {
  ...
  function save() {
    // Factors are now derived from the structured diary answers so the
    // chips that show on the timeline / past-day stay in sync.
    const factors = factorsFromDiary(diary);
    update(entry!.id, {
      moodX, moodY,
